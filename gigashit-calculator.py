...
Label(text="CPU Single Core Performance (Find it out in website below)").grid(
    column=1, row=2)
singlecoreentry = Entry()
singlecoreentry.grid(column=1, row=3)

cpulink = Label(text="cpubenchmark.net", font=(
    'Arial Bold', 10), fg="blue", cursor="hand2")
...
Label(text="Ping to ISP (ms) (Be sure to choose your ISP's closest server to you)").grid(
    column=1, row=27)
pingentry = Entry()
pingentry.grid(column=1, row=28)
pinglink = Label(text="speedtest.net", font=(
    'Arial Bold', 10), fg="blue", cursor="hand2")
...
def clicked():
    ...
    # Calculating outputs
    shits = ((((singlecore/2740*corecount*clockspeed)/2*(ramamount*ramspeed)/2))
             * 1000+(storageread+storagewrite)/2*(internetdown*internetup)/ping)*10
    ...
    if ogshits > 1000:
        ...
        if kiloshits > 1000:
            if megashits > 1000:
                if gigashits > 1000:
                    if terashits > 1000:
                        metric1results.configure(
                            text=str(round(petashits, 2))+" PS (Petashits)")
                    else:
                        metric1results.configure(
                            text=str(round(terashits, 2))+" TS (Terashits)")

                else:
                    metric1results.configure(
                        text=str(round(gigashits, 2))+" GS (Gigashits)")
            else:
                metric1results.configure(
                    text=str(round(megashits, 2))+" MS (Megashits)")
        else:
            metric1results.configure(
                text=str(round(kiloshits, 2))+" KS (Kiloshits)")


# def CreateLabel(newText, newColoumn, newRow):
# global newLabel
# newLabel = Label(text=newText)
# global z
# z = newLabel.grid(column=newColoumn, row=newRow)
# return newLabel


# def pressed():
# CreateLabel("eisai pouttos", 10, 2)
...
# pouttosini += 1
# print(pouttosini)
# newLabel.grid(column=pouttosini, row=2)

labelNames = ["eisai pouttos",
              "oi i mana sou en pouttos", "papas sou", "giagia sou"]
labelArray = []
btn = Button(text="Click here to calculate your shits™",
             command=clicked).grid(column=1, row=30)


def GenerateLabel(text, newRow, newColumn):
    newLabel = Label(text=text).grid(row=newRow, column=newColumn)
...
for name in labelNames:
    GenerateLabel(name, CurrentRow, CurrentColumn)
    CurrentRow += 1
